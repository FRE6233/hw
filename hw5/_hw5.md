...
Since $B_{t_j}$ and $\Delta B_j$ are independent we have
$E[B_{t_j}\Delta B_j] = E[B_{t_j}]E[\Delta B_j] = 0$
</details>
...
<details><summary>Solution</summary>
Since $E[B_t B_u] = \min\{t, u\}$ we have
$E[B_{t_{j+1}}\Delta B_j]
= E[B_{t_{j+1}}(B_{j+1} - B_j]
= t_{j+1} - t_j = \Delta t_j$
Since $B_u - B_t$ has the same distribution as $B_{u - t}$.
</details>
...
<details><summary>Solution</summary>
We have $E[(\Delta B_j)^2] = \Var(\Delta B_j) = \Delta t_j$
Since $\Delta B_j$ has the same distribution as $B_{t_{j + 1}} - B_{t_j}$.
</details>
...
<details><summary>Solution</summary>
This follows since $t$ is finite and $\max_j \Delta t_j$ goes to 0.
</details>
...
<details><summary>Solution</summary>
The moment generating function is $E[e^{s\Delta B}] = e^{s^2\Delta t/2}$.
Using $e^{sx} = \sum_{n=0}^\infty (sx)^n/n!$ and matching fourth moments,
$E[(\Delta B)^4]/4! = (\Delta t/2)^2/2!$ and $E[(\Delta B)^4] = 3(\Delta t)^2$.

We have $\Var[\sum_{0\le j < n} (\Delta B_j)^2] = \sum_{0\le j < n} 3(\Delta t_j)^2$
which goes to 0 based on the previous exercise.
</details>
...
<details><summary>Solution</summary>
From the previous exercise, $\Var(\Delta B_j \Delta t_j) = 3(\Delta t_j)^3$.
Since $x^3 < x^2$ if $|x| < 1$, this goes to zero even faster.
</details>
...
<details><summary>Solution</summary>
Let $M_t = \int_0^t \sigma\,dB$. We must show $E[M_u\mid\mathcal{F}_t] = M_t$.
We write $E_t[X]$ for $E[X\mid\mathcal{F}_t]$
$$
\begin{aligned}
E_t[M_u] &= E_t[\int_0^u \sigma\,dB] \\
	&= E_t[\int_0^t \sigma\,dB + \int_t^u \sigma\,dB] \\
	&= E_t[\int_0^t \sigma\,dB] + E_t[\int_t^u \sigma\,dB] \\
	&= E_t[\int_0^t \sigma\,dB] \\
	&= \int_0^t \sigma\,dB \\
	&= M_t \\
\end{aligned}
$$
...
<details><summary>Solution</summary>
We have $X_t = E_t[X_u] = E_t[\int_0^u \mu\,ds + \int_0^u \sigma\,dB_s]$ so
$\int_0^t \mu \,ds = E_t[\int_0^u \mu\,ds]$.
</details>
...
<details><summary>Solution</summary>
By Ito's formula $df(t, B_t) = f_t(t, B_t)\,dt + f_x(t, B_t)\,dB + (1/2)f_{xx}(t, B_t)\,(dB)^2
= (f_t + (1/2)f_{xx})\,dt + f_x\,dB$. This is a martingale if and only if
...
<details><summary>Solution</summary>
If $f(t, x) = x^2 - t$ then $f_t = -1$, $f_x(t,x) = 2x$, and $f_{xx}(t, x) = 2$ so
$f_t + (1/2)f_{xx} = -1 + 1 = 0$.

If $f(t, x) = e^{-x^2/2t}/\sqrt{2\pi t}$ then
$f_t = (\sqrt{2\pi t}((-x/t)e^{-x^2/2t}) - (e^{-x^2/2t}(1/2)2\pi/\sqrt{2\pi t}))/2\pi t$
</details>
